...
The entire server.py is generated by an LLM (Gemini or DeepSeek)
in a single pass. No scaffolding, no stitching — pure generation.
Validated with ast.parse() and auto-repaired if needed.
Generated servers use mcp.server.fastmcp.FastMCP.

Provider priority: Gemini (GEMINI_API_KEY) → Featherless (FEATHERLESS_API_KEY)
"""
...
def _call_llm(prompt: str, *, system_instruction: str | None = None,
    ...
    """Call LLM — Gemini (primary) or Featherless (fallback)."""
    logger = get_logger()
    ...
    if gemini_key:
        return _call_gemini_codegen(prompt, gemini_key, system_instruction, temperature, max_tokens, logger)
    elif featherless_key:
        return _call_featherless(prompt, featherless_key, system_instruction, temperature, max_tokens, logger)
    else:
        raise RuntimeError(
            "No LLM API key found. Set GEMINI_API_KEY or FEATHERLESS_API_KEY in .env"
        )


def _call_gemini_codegen(prompt, api_key, system, temperature, max_tokens, logger):
    """Call Google Gemini API for code generation."""
    logger.info("  LLM → Gemini (gemini-2.5-flash)")
...
def _call_featherless(prompt, api_key, system, temperature, max_tokens, logger):
    """Call DeepSeek-V3 via Featherless API (fallback)."""
...
_SYSTEM_PROMPT = textwrap.dedent("""\
    ...
    MCP server files using the `mcp.server.fastmcp.FastMCP` framework.
...
def _build_server_prompt(
    ...
    return textwrap.dedent(f"""\
        ...
        - For EVERY tool you generate, you MUST add an optional parameter `auth_token: str | None = None`.
        - If `auth_token` is provided, use it in the `Authorization: Bearer {{auth_token}}` header instead of the default `API_KEY`. This is necessary for deep agent security testing to override the token dynamically.
